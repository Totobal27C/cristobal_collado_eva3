...
export default function LoginPage() {
  ...
  const validatePasswordMatch = () => {
    if (!form.confirmPassword) return 'Debes repetir la contraseña'
    ...
    <div className="container" style={{ maxWidth: '420px', margin: '3rem auto' }}>
      <div className="card">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {success.email && <div className="success-msg show">{success.email}</div>}
          ...
          {success.confirmPassword && <div className="success-msg show">{success.confirmPassword}</div>}

          <button type="submit">Entrar</button>
          <p style={{ textAlign: 'center' }}><Link to="/nuevo-usuario">¿No tienes cuenta?</Link></p>
        </form>
      </div>
    </div>
  )
